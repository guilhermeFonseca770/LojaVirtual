$(document).ready(function(){
		...
		$.ajax({
			...
			success : function(response){
				...
				if (resp.status == 202) {
					...
					if (productList) {
						$.each(resp.message.products, function(index, value){
							...
							productHTML += '<tr>'+
								              ...
								              '<td><img width="60" height="60" src="/flawers/product_images/'+value.product_image+'"></td>'+
					...
					var catSelectHTML = '<option value="">Selecione a Categoria</option>';
					...
					var brandSelectHTML = '<option value="">Selecione a Marca</option>';
	...
	$(document.body).on('click', '.edit-product', function(){
		...
		$("input[name='e_product_image']").siblings("img").attr("src", "/flawers/product_images/"+product.product_image);
