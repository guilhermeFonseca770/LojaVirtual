...
class Products
{
	...
	public function addProduct($product_name,
		...
		if ($ext == "jpg" || $ext == "jpeg" || $ext == "png") {
			...
			if ($file['size'] > (1024 * 2)) {
				...
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/flawers/product_images/".$uniqueImageName)) {
					...
					if ($q) {
						return ['status'=> 202, 'message'=> '❀ Produto cadastrado com sucesso! ❀'];
					}else{
				...
				return ['status'=> 303, 'message'=> '❀ Tamanho da image muito grande ,Maximo 2MB! ❀'];
			}

		}else{
			return ['status'=> 303, 'message'=> '❀ Formato da imagem invalido [Formatos validos : jpg, jpeg, png] ❀'];
		}
	...
	public function editProductWithoutImage($pid,
		...
		if ($pid != null) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> '❀ Produto Alterado com sucesso! ❀'];
			}else{
				return ['status'=> 303, 'message'=> '❀ Falha na atualização! ❀'];
			}
			
		}else{
			return ['status'=> 303, 'message'=> '❀ ID do produto invalido! ❀'];
		}
	...
	public function addCategory($name){
		...
		if ($q->num_rows > 0) {
			return ['status'=> 303, 'message'=> '❀ Categoria Ja existente! ❀'];
		}else{
			...
			if ($q) {
				...
				return ['status'=> 303, 'message'=> '❀ Falha ao adicionar categoria, verifique o nome! ❀'];
			}
		}
	}
	...
	public function deleteProduct($pid = null){
		if ($pid != null) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> '❀ Produto Removido do Estoque! ❀'];
			}else{
				return ['status'=> 202, 'message'=> '❀ Falha ao remover item do estoque! ❀'];
			}
			
		}else{
			return ['status'=> 303, 'message'=>'❀ ID do Produto é invalido! ❀'];
		}
	...
	public function deleteCategory($cid = null){
		if ($cid != null) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> '❀ Categoria excluida! ❀'];
			}else{
				return ['status'=> 202, 'message'=> '❀ Falha ao remover categoria! ❀'];
			}
			
		}else{
			return ['status'=> 303, 'message'=>'❀ ID da Categoria é invalido! ❀'];
		}
	...
	public function updateCategory($post = null){
		...
		if (!empty($cat_id) && !empty($e_cat_title)) {
			...
			if ($q) {
				...
				return ['status'=> 202, 'message'=> '❀ Falha ao Atualizar Categoria! ❀'];
			}
	...
	public function deleteBrand($bid = null){
		if ($bid != null) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> '❀ Marca ecluida! ❀'];
			}else{
				return ['status'=> 202, 'message'=> '❀ Falha ao excluir marca! ❀'];
			}
			
		}else{
			return ['status'=> 303, 'message'=>'ID da marca é invalido! ❀'];
		}
	...
	public function updateBrand($post = null){
		...
		if (!empty($brand_id) && !empty($e_brand_title)) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> '❀ Marca atualizada com sucesso! ❀'];
			}else{
				return ['status'=> 202, 'message'=> '❀ Falha ao atualizar marca! ❀'];
			}
...
if (isset($_POST['add_product'])) {
	...
	&& !empty($_FILES['product_image']['name'])) {
		...
		echo json_encode(['status'=> 303, 'message'=> '❀ Há campos vazios! Cheque todos! ❀']);
		exit();
	}
...
if (isset($_POST['add_category'])) {
	if (isset($_SESSION['admin_id'])) {
		...
		echo json_encode(['status'=> 303, 'message'=> '❀ Erro na sessão ❀']);
	}
}
...
if (isset($_POST['DELETE_PRODUCT'])) {
	...
	if (isset($_SESSION['admin_id'])) {
		if(!empty($_POST['pid'])){
			...
			echo json_encode(['status'=> 303, 'message'=> '❀ ID do produto invalido! ❀']);
			exit();
		}
	}else{
...
if (isset($_POST['DELETE_CATEGORY'])) {
	if (!empty($_POST['cid'])) {
		...
		echo json_encode(['status'=> 303, 'message'=> '❀ Detalhes invalidos! ❀']);
		exit();
	}
}
