	...
	public function loginAdmin($email, $password){
		...
		if ($q->num_rows > 0) {
			...
			if (password_verify($password, $row['password'])) {
				...
				return ['status'=> 202, 'message'=> '❀ Logando no Sistema! ❀'];
			}else{
				return ['status'=> 303, 'message'=> '❀ Dados incorretos! ❀'];
			}
		}else{
			return ['status'=> 303, 'message'=> '❀ Este e-mail não esta cadastrado! ❀'];
		}
	}
...
if (isset($_POST['admin_register'])) {
	...
	if (!empty($name) && !empty($email) && !empty($password) && !empty($cpassword)) {
		...
		echo json_encode(['status'=> 303, 'message'=> '❀ Há campos vazios! ❀']);
		exit();
	}
}
